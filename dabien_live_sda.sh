...
mydev=$1
fdisk -l $mydev && export dev="${mydev}" 
zenity --title "Approve device"  --question --text "Scanned device set to $dev - continue?" || exit 0
...
zenity --title "Approve device"  --question --text "This nukes data on ${dev} - continue?" || exit 0
...
xterm -T "Encrypting partition - insert passphrase" -e "cryptsetup luksFormat ${dev}2 " && xterm -T "Opening newly encrypted partition" -e "cryptsetup open ${dev}2 dev2" && mkfs.ext4 -F -L persistence /dev/mapper/dev2  && sync
sleep 3
partprobe
echo "50" ; sleep 1
...
rmdir ${tmpall} ${tmp}
echo "95" ; sleep 1
echo "# Sync'ing so that cached writes to stick are written"
sync
echo "100" ; sleep 1
echo "#Install is ready" 
) |
zenity --progress --title "Installing the key to device" \
