...
mkdir -p $HOME/liveimages-bookworm-games/dabienkey
cd $HOME/liveimages-bookworm-games/dabienkey
mkdir -p $desktop
cd $desktop/
lb clean --binary
...
echo  >> config/package-lists/installer.list.chroot
...
echo doomsday circuslinux bugsquish bomberclone brainparty chromium-bsu davegnukem extremetuxracer freedroid freetennis galois games-all gcompris-qt gltron granatier gravitywars kapman liquidwar madbomber luola neverputt neverball open-invaders pink-pony tuxtype supertux supertuxkart xgalaga  >> config/package-lists/installer.list.chroot
