...
mkdir -p /liveimages/dabienkey-unstable
cd /liveimages/dabienkey-unstable
mkdir -p $desktop
cd $desktop/
lb clean --binary
...
echo myspell-da myspell-de-de myspell-fr myspell-es myspell-sv-se myspell-nb  myspell-fo myspell-et myspell-lv myspell-nn  myspell-ru >> config/package-lists/installer.list.chroot
...
echo evolution-ews >> config/package-lists/installer.list.chroot
...
echo rsync rsnapshot net-tools cryptsetup  >> config/package-lists/installer.list.chroot
...
echo meld emacs  >> config/package-lists/installer.list.chroot
...
echo pidgin pidgin-sipe tigervnc-viewer >> config/package-lists/installer.list.chroot
echo flatpak gnome-software-plugin-flatpak >> config/package-lists/installer.list.chroot
...
# new win10'ish desktop 
  wget --no-check-certificate -O skel.tgz https://github.com/linuxuser42/dabien/raw/master/dabien_buster_gnome2panel.tar.gz
